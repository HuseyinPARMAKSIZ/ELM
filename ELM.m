...
function  [TrainingTime, TestingTime, TrainingAccuracy, TestingAccuracy]= ELM(TrainingData_File, TestingData_File, Elm_Type, NumberofHiddenNeurons, ActivationFunction,C,ite)
...
end_time_train=toc(start_time_train);%cputime;
...
disp('------------- ELM ------------- ');
disp('[  RegFac.   Nnode     TrAcc.       TeAcc.    TrTime  TeTime  ActivationFunc.  ]');
XX=[           C     NumberofHiddenNeurons  TrainingAccuracy   TestingAccuracy       TrainingTime   mean(time) string(ActivationFunction)];
disp(XX);
end
